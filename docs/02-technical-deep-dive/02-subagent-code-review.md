...
| **置信度** | AI对自己给出的结论有多确定，用0到1的归一化分数表示 | 就像天气预报的降水概率。置信度0.90意味着"非常确定"，0.30意味着"不太靠谱" |
...
│                              ▼                                      │
│   输出：最终评审意见                                                  │
│   - 🔴 必须修改：食材有过敏原（安全员，0.95）                          │
│   - 🟡 建议改进：摆盘可以更精致（规范师，0.88）                        │
│   - ⚪ 效率师的意见被过滤（置信度不够）                                │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
```
  ...
  │  各专家提交评审结果                                           │
  │                                                             │
│  安全专家:  置信度 0.95  ████████████████████░░  发现3个问题   │
│  性能专家:  置信度 0.72  ██████████████░░░░░░░░  发现2个问题   │
│  规范专家:  置信度 0.88  █████████████████░░░░░  发现5个问题   │
  │                                                             │
  └──────────────────────────┬──────────────────────────────────┘
                             │
                             ▼
...
│                                                             │
│  安全专家:  0.95 ≥ 0.80  ✅ 通过                               │
│  性能专家:  0.72 < 0.80  ❌ 过滤 → 记录到统计，不进入最终报告     │
│  规范专家:  0.88 ≥ 0.80  ✅ 通过                               │
  │                                                             │
  │  为什么设0.80？                                               │
  │  • 太低（如0.60）：太多不靠谱的建议混入报告，浪费开发者时间      │
  │  • 太高（如0.95）：有价值的中等置信度建议也被过滤，漏报风险增大    │
  │  • 0.80是平衡点：保留大部分靠谱建议，过滤掉明显不靠谱的          │
  │                                                             │
  └──────────────────────────┬──────────────────────────────────┘
                             │
                             ▼
  ...
  │                                                             │
  │  🔴 高危问题（必须修复）                                       │
  │     1. SQL注入风险 (第42行, 置信度0.95)                         │
  │                                                             │
  │  🟡 中危问题（建议修复）                                       │
  │     2. 缺少JSDoc文档 (第15行, 置信度0.88)                       │
  │                                                             │
  │  🟢 低危问题（可选优化）                                       │
  │     3. 变量命名不规范 (第8行, 置信度0.88)                        │
  │                                                             │
  │  📊 统计信息                                                  │
  │     • 总发现数: 10 → 过滤后: 3                                │
  │     • 被过滤: 7 (性能专家0.72未达标)                            │
...
interface AggregatorConfig {
  minConfidenceThreshold: number;  // 最低置信度阈值（默认0.80）
...
3. **置信度阈值设为0.80的依据是什么？** 如果设为0.50或0.95，分别会有什么后果？
