...
> 深度解析Claude Code SDK的架构设计与核心实现机制  
> 难度：⭐⭐⭐⭐⭐  
> 核心概念：MCP协议、工具发现、状态机调度、Session管理
...
| 工具描述格式不统一，LLM难以理解 | JSON Schema标准描述，LLM直接读懂 |
...
MCP是Claude Code SDK的核心协议，定义了一套**标准化的工具接口**，让LLM能够：
- **动态发现**环境中的可用工具
- **理解**工具的输入参数和输出格式
- **调用**工具并处理返回结果
...
interface MCPTool {
  ...
  parameters: JSONSchema;          // 参数模式（JSON Schema）
  returns: JSONSchema;             // 返回模式
...
interface ToolCallRequest {
  ...
  callId: string;                  // 调用ID（用于关联结果）
}

// 3. 工具调用结果
interface ToolCallResult {
  callId: string;                  // 对应请求的ID
  status: 'success' | 'error';     // 调用状态
  result?: any;                    // 成功时的结果
  error?: ToolError;               // 失败时的错误
  metadata: {
...
> 💡 **白话理解**：下面这段代码就是在写"菜单"。`readFileTool`就是菜单上的一道菜，名叫"read_file"，描述是"读取文件内容"，参数是"文件路径"和"读取行数"，返回值是"文件内容"。LLM看到这个菜单，就知道该怎么点菜了。
...
interface ReActStep {
  ...
  state: ReActState;
  thought?: string;            // 思考内容
  action?: ToolCallRequest;    // 执行的动作
  observation?: ToolCallResult; // 观察结果
  timestamp: Date;
}
  ...
  private formatConversationHistory(): string {
    ...
    return this.steps.map(step => {
      if (step.thought) {
        ...
        return `行动: 调用 ${step.action.tool}`;
      } else if (step.observation) {
        return `观察: ${JSON.stringify(step.observation.result)}`;
      }
      return '';
    }).filter(Boolean).join('\n');
  }
...
interface Message {
  ...
  timestamp: Date;
  metadata?: {
    toolCall?: ToolCallRequest;   // 工具调用信息
    toolResult?: ToolCallResult;  // 工具返回结果
...
> 💡 **白话理解**：上下文压缩就像你读一本很长的书。你不可能逐字记住每一页，所以你会对前面的章节做个摘要（"第一章讲了主角出生，第二章讲了主角上学"），然后对最近几页逐字细读。这样既不会丢失关键信息，又不会超出你的记忆容量。SessionManager做的就是这个工作：当对话太长时，把早期消息压缩成摘要，只保留最近的消息原文。
...
### 2. 协议层面
"SDK基于**MCP（Model Context Protocol）协议**，定义了：
- 工具的标准化描述格式（JSON Schema）
- 工具发现机制（动态加载）
- 调用和返回的数据格式"
...
2. **MCP的四个核心组件（工具定义、调用请求、调用结果、服务器接口）分别对应USB协议中的什么？** 提示：想想设备描述、数据传输、状态反馈、设备驱动。
...
> **下一篇**：[SDK轻量化重构与多租户架构](../03-architecture-design/01-multi-tenant-architecture.md)