...
> 基于真实AI应用场景的深度技术文档，涵盖Agent架构、MCP协议、Claude Code SDK、Subagent协作、多租户系统等核心知识点  
...
| **状态机** | 一种设计模式，把系统分成若干个状态，每个状态下有特定的行为和转换规则。状态之间的转换由事件触发，确保系统行为可预测 | 像红绿灯：红灯→绿灯→黄灯→红灯，每个状态对应不同行为，转换有固定规则 |
...
| **Harness** | 一种抽象层，把底层复杂实现封装起来，对外提供统一接口。方便替换底层组件而不影响上层业务逻辑，是SDK轻量化的关键设计 | 像电源适配器，不管你插的是110V还是220V，输出给电脑的都是稳定的电压 |
...
| ⭐⭐⭐⭐ | 🏢 [SDK轻量化与多租户架构](./03-architecture-design/01-multi-tenant-architecture.md) | Harness抽象+分布式部署 | 45min | 多租户、Harness、上下文压缩 |
...
| Day 17 | Harness | 理解Harness抽象层的设计思想 | 用类比解释Harness的作用 |
...
2. **Chain-of-Thought**: Large Language Models are Zero-Shot Reasoners
   - 核心观点：让AI写出推理过程，准确率显著提升
   - 阅读建议：重点看"Let's think step by step"这个prompt的实验
...
- [Claude Code](https://github.com/anthropics/claude-code) - 官方SDK，本知识库的核心参考
